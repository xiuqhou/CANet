...
from models.backbones.plugins import PluginConfig
...
from util.tune_mode_convbn import turn_on_efficient_conv_bn_eval_for_single_model
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        conv2 = conv3x3_dcn if with_dcn else conv3x3
        self.conv2 = conv2(width, width, stride, groups, dilation)
        ...
        if self.with_plugins:
            for i, in_channels in enumerate([width, width, planes * self.expansion]):
                block_plugins = [p.block(in_channels) for p in plugins if i in p.conv_pos]
                self.register_module(f"plugin{i+1}", nn.Sequential(*block_plugins))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.with_plugins:
            out = self.plugin2(out)

        out = self.conv3(out)
        out = self.bn3(out)

        if self.with_plugins:
            out = self.plugin3(out)
...
class ResNet(nn.Module):
    def __init__(
        ...
        # build layers with plugins
        if plugins is None:
            layer_plugins = [None for _ in range(4)]
        else:
            assert all([isinstance(p, PluginConfig) for p in plugins]), \
                "plugins should be a list of PluginConfig"
            layer_plugins = [[p for p in plugins if i in p.stages] for i in range(4)]

        self.layer1 = self._make_layer(
            block,
            64,
            layers[0],
            with_dcn=stage_with_dcn[0],
            plugins=layer_plugins[0],
        )
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep normalization layer in eval mode."""
...
class ResNetBackbone(BaseBackbone):
    ...
    model_weights = {
        ...
        "resnext50_32x4d_v1": ResNeXt50_32X4D_Weights.IMAGENET1K_V1,
        "resnext50_32x4d_v2": ResNeXt50_32X4D_Weights.IMAGENET1K_V2,
        "resnext101_32x8d_v1": ResNeXt101_32X8D_Weights.IMAGENET1K_V1,
        "resnext101_32x8d_v2": ResNeXt101_32X8D_Weights.IMAGENET1K_V2,
        "resnext101_64x4d": ResNeXt101_64X4D_Weights.IMAGENET1K_V1,
        "wide_resnet50_2_v1": Wide_ResNet50_2_Weights.IMAGENET1K_V1,
        "wide_resnet50_2_v2": Wide_ResNet50_2_Weights.IMAGENET1K_V2,
        "wide_resnet101_2_v1": Wide_ResNet101_2_Weights.IMAGENET1K_V1,
        "wide_resnet101_2_v2": Wide_ResNet101_2_Weights.IMAGENET1K_V2,
    ...
    def __new__(
        ...
        **kwargs,
        ...
        # fuse conv bn
        if fuse_conv_bn:
            turn_on_efficient_conv_bn_eval_for_single_model(resnet)
...
if __name__ == "__main__":
    ...
    dummy_input = torch.ones(2, 3, 128, 128)
