...
from transforms import detr_transform as TT
...
from transforms.detr_transform import DETRTransformsWrapper
...
supp_scales = [448, 464, 480, 496, 512, 528, 544, 560, 576, 592, 608, 624, 640, 656, 672]
detr_support = T.Compose([
    T.RandomHorizontalFlip(),
    T.RandomChoice([
        T.RandomShortestSize(min_size=supp_scales, max_size=672, antialias=True),
        T.Compose([
            T.RandomShortestSize([400, 500, 600], antialias=True),
            RandomSizeCrop(384, 600),
            T.RandomShortestSize(min_size=supp_scales, max_size=672, antialias=True),
...
detr300 = T.Compose([
    ...
    T.RandomChoice([
        T.RandomShortestSize(
            min_size=[int(0.25 * s) for s in scales], max_size=300, antialias=True
        ),
        T.Compose([
            T.RandomShortestSize([100, 125, 150], antialias=True),
            RandomSizeCrop(100, 150),
            T.RandomShortestSize(
                min_size=[int(0.25 * s) for s in scales], max_size=300, antialias=True
            ),
        ]),
...
detr600 = T.Compose([
    ...
    T.RandomChoice([
        T.RandomShortestSize(
            min_size=[int(0.5 * s) for s in scales], max_size=600, antialias=True
        ),
        T.Compose([
            T.RandomShortestSize([200, 250, 300], antialias=True),
            RandomSizeCrop(200, 300),
            T.RandomShortestSize(
                min_size=[int(0.5 * s) for s in scales], max_size=600, antialias=True
            ),
        ]),
...
strong_album_2048 = T.Compose([
    T.RandomChoice([
        T.RandomShortestSize(
            min_size=[1600, 1760, 1800, 1920, 2000, 2048],
            max_size=2048,
            antialias=True,
        ),
        T.Compose([
            T.RandomShortestSize([1600, 1800, 2000], antialias=True),
            RandomSizeCrop(1500, 2000),
            T.RandomShortestSize(
                min_size=[1600, 1760, 1800, 1920, 2000, 2048],
                max_size=2048,
                antialias=True,
