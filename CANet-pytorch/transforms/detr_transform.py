...
def crop(image, target, region):
    ...
    fields = [f for f in ["labels", "area", "iscrowd"] if f in target.keys()]
...
def resize(image, target, size, max_size=None):
    ...
    if isinstance(rescaled_image, PIL.Image.Image):
        rescaled_image_size = rescaled_image.size
    elif isinstance(rescaled_image, torch.Tensor):
        _, h, w = rescaled_image.shape
        rescaled_image_size = (w, h)
...
class DETRTransformsWrapper(nn.Module):
    def __init__(self, detr_transforms):
        """A wrapper for transforms from DETR repository

        :param detr_transforms: transforms from DETR repository
        """
        super().__init__()
        self.detr_transforms = detr_transforms
    ...
    def forward(self, input: Any) -> Any:
        ...
        images = T.ToPILImage()(images[0].data)
        targets = {
            "boxes": boxes[0].data,
            "labels": labels,
        }
        if len(masks) != 0:
            targets.update({"masks": masks[0].data})
        images, targets = self.detr_transforms(images, targets)

        # prepare output data format
        images = datapoints.Image(T.PILToTensor()(images))
        boxes = datapoints.BoundingBox(
            torch.as_tensor(targets["boxes"]).reshape(-1, 4),  # in case of empty boxes after transforms
            dtype=torch.float,
