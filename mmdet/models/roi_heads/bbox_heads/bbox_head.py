...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.roi_feat_size = _pair(roi_feat_size) # RoI尺寸为7*7
        self.roi_feat_area = self.roi_feat_size[0] * self.roi_feat_size[1] # RoI面积为49
        self.in_channels = in_channels  # 经过FPN的输入特征图通道数
        self.num_classes = num_classes  # 待预测类别数
        ...
        if self.with_avg_pool:
            ...
            in_channels *= self.roi_feat_area   # 256 * 49，为什么要这么做？
        if self.with_cls:
            ...
            self.fc_cls = build_linear_layer(
                ...
                in_features=in_channels,    # in_channels * roi_area -> num_classes + 1
                out_features=cls_channels)
        if self.with_reg:
            out_dim_reg = 4 if reg_class_agnostic else 4 * num_classes # num_classes * 4
