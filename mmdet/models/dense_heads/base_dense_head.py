...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def forward_train(self,
        ...
        """
        outs = self(x) # 内部分别经过两个简单的卷积，返回预测的cls_score和bbox_pred，长度均为batch_size，维度分别为[9, hl, wl]和[9*4, hl, wl]
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore) # 输入预测的cls_score、bbox_pred、gt_bboxes、img_metas信息
