...
def train_detector(model,
    ...
    cfg = compat_cfg(cfg) # 对cfg进行一些处理，警告过时的配置
    logger = get_root_logger(log_level=cfg.log_level) # 构建日志记录器

    # prepare data loaders
    dataset = dataset if isinstance(dataset, (list, tuple)) else [dataset]

    runner_type = 'EpochBasedRunner' if 'runner' not in cfg else cfg.runner[ # 默认的runner为EpochBasedRunner
        'type']

    train_dataloader_default_args = dict( # 构建数据加载器
        samples_per_gpu=2,
        workers_per_gpu=2,
    ...
    } # 将train_dataloader_default_args参数和cfg配置文件中的train_loader部分通过{}集合进行合并

    data_loaders = [build_dataloader(ds, **train_loader_cfg) for ds in dataset] # 构建数据加载器，内部采用GroupSampler来实现
    ...
    # build optimizer
    auto_scale_lr(cfg, distributed, logger) # 自动缩放学习率
    ...
    runner.timestamp = timestamp # 记录当前时间节点
    ...
    runner.run(data_loaders, cfg.workflow) # 开始工作流程
