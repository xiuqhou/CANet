...
class AnchorGenerator:
    ...
    def __init__(self,
        ...
        self.base_anchors = self.gen_base_anchors() # 多尺度的特征图，一个列表
    ...
    def gen_single_level_base_anchors(self,
        ...
        w_ratios = 1 / h_ratios
        if self.scale_major: # 这两种生成方式排列方式不同，scale_major生成的锚框在同一行会有相同的尺寸
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device='cuda'):
        ...
        return multi_level_anchors # 列表包含每张特征图的锚框[hl*wl*base_anchor_num,4] 注意没有batch
    ...
    def single_level_grid_priors(self,
        ...
        base_anchors = self.base_anchors[level_idx].to(device).to(dtype) # [9,4]
        ...
        all_anchors = base_anchors[None, :, :] + shifts[:, None, :] # [41600, 9, 4]
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        assert self.num_levels == len(featmap_sizes)
        multi_level_flags = []
        for i in range(self.num_levels): # 对于单张图片的每个特征图
            anchor_stride = self.strides[i]
            feat_h, feat_w = featmap_sizes[i]
            h, w = pad_shape[:2]
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h) # 对特征图取整
