...
def draw_labels(ax,
    ...
    for i, (pos, label) in enumerate(zip(positions, labels)):
        ...
        ax.text(
            ...
            verticalalignment='bottom', # 原本为top，改为下面对齐？
...
def imshow_det_bboxes(img,
    ...
    """此处为我的修改，修改字体尺寸和检测框粗细"""
    font_size=np.floor(0.1 * np.shape(img)[1] + 0.5) # 原本是0.03
    thickness=thickness = max((np.shape(img)[0] + np.shape(img)[1]) // 280, 1) # 原本是1080
    """"""
    ...
    if bboxes is not None:
        ...
        draw_bboxes(ax, bboxes, colors, alpha=0.8, thickness=thickness) # 画bbox框

        horizontal_alignment = 'left'
        positions = bboxes[:, :2].astype(np.int32) # label的起始位置，这里要修改，原本为+thickness
        areas = (bboxes[:, 3] - bboxes[:, 1]) * (bboxes[:, 2] - bboxes[:, 0]) # bbox的面积
        scales = _get_adaptive_scales(areas) # 每个框的缩放尺度，改为全部都是1？
        scales[:] = 0.5
...
def imshow_gt_det_bboxes(img,
    ...
    img = mmcv.imread(img)
    """此处为我的修改，修改字体尺寸和检测框粗细"""
    font_size=np.floor(0.03 * np.shape(img)[1] + 0.5)
