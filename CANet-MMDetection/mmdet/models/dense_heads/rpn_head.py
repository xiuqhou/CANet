...
class RPNHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        if self.num_convs > 1:
            ...
            self.rpn_conv = nn.Conv2d(
                self.in_channels, self.feat_channels, 3, padding=1) # 特征图变换256 -> 256
        self.rpn_cls = nn.Conv2d(self.feat_channels,
                                 self.num_base_priors * self.cls_out_channels, # 256 -> 9 * 1
                                 1)
        self.rpn_reg = nn.Conv2d(self.feat_channels, self.num_base_priors * 4, # 256 -> 9 * 4
                                 1)
    ...
    def loss(self,
        ...
        """
        losses = super(RPNHead, self).loss( # 直接调用AnchorHead的损失计算
            cls_scores,
            bbox_preds,
            gt_bboxes,
            None,
