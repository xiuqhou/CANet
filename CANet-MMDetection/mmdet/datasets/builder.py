...
def build_dataloader(dataset,
    ...
    if dist:
        ...
        batch_size = num_gpus * samples_per_gpu # batchsize是gpu数目*每个gpu的样本数
        num_workers = num_gpus * workers_per_gpu # num_workers线程数是gpu数目*每个gpu的线程数
    ...
    if runner_type == 'IterBasedRunner':
        ...
        if class_aware_sampler is not None:
            ...
            sampler = GroupSampler(dataset, # 对数据集进行Group采样，使形状相近的数据尽可能划分在同一组
                                   samples_per_gpu) if shuffle else None
        batch_sampler = None # sampler.num_samples=1344为什么这么大？
