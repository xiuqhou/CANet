...
model = dict(
    ...
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
                type='MaxIoUAssigner',
                pos_iou_thr=0.7,  # NOTE: 这里降低阈值看看效果，可能0.5时比较好的结果
                neg_iou_thr=0.3,
                min_pos_iou=0.3,
...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=(1333, 1333), keep_ratio=True),
...
data_root = 'data/mobile_screen_zcf_coco/'
dataset_type = 'CocoDataset'
classes = ('pinhole', 'scratch', 'bubble', 'tin_ash') # , 'other', 'pinhole_a', '1'
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=1,
    train=dict(
