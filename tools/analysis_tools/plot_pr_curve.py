...
def plot_pr_curve(config_file, result_file, metric="bbox",save_dir=None):
    ...
    plt.savefig(save_dir+'PR_curve.jpg')

def parse_args():
    parser = argparse.ArgumentParser(
        description='MMDet plot pr curve for CoCo')
    parser.add_argument('config', help='test config file path')
    parser.add_argument(
        'result', help='prediction path where test pkl result')
    parser.add_argument(
...
if __name__ == "__main__":
    args = parse_args()
    plot_pr_curve(config_file=args.config, result_file=args.result, save_dir=args.show_dir)